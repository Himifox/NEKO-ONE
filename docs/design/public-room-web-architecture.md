...
> 状态：提案（用于在现有 N.E.K.O 代码基础上裁剪第一版）  
> 日期：2026-08-15  
...
| 故障 | 对外行为 | 恢复策略 |
|---|---|---|
| LLM 超时/限流 | `stream.failed`，保留访客消息 | 指数退避；可切备用供应商；不自动重复公开回复 |
| Memory 不可用 | 使用 Persona 缓存和最近消息继续 | 熔断、后台补写，标记 memory-degraded |
| TTS 失败 | 文字正常，提示音频不可用 | jobs 重试；达到上限后人工查看 |
...
以下问题不改变本文总体架构，但应在 Phase 0 各自形成简短 ADR：

1. 保留 Python/FastAPI 的前提下，`LLMSessionManager` 拆分范围与 provider 适配器归属；
2. 匿名 Cookie 身份是否允许长期记忆，及其默认保留期；
3. 账号登录供应商和管理员 MFA 方案；
4. 内容审核供应商、失败时 fail-closed 的具体范围；
5. Live2D 模型、动作、字体和 TTS 音色的公网使用许可；
6. 公共时间线和原始聊天日志的保留/删除周期；
7. 现有 Memory Server 的内部 API 如何扩展三层作用域，以及旧 `master` 数据如何拆分；
8. TTS 采用整句生成还是稳定句段流水线，以及浏览器自动播放体验；
9. 博客使用链接、同域反代还是受限 iframe 嵌入；
10. 现有模板与 `frontend/react-neko-chat` 选择哪一个作为唯一前端源码，另一个何时删除。

在这些 ADR 完成前可以搭协议与房间骨架，但不得默认匿名访客永久记忆、公开音色或可无限保留聊天内容。
